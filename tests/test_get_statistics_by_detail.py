...
def mock_repositories():
    ...
    repo1.get_pulls.side_effect = lambda state: MagicMock(totalCount=4) if state == "open" else MagicMock(totalCount=1) if state == "closed" else [MagicMock(user=MagicMock(login="dependabot1"))]
    ...
    repo2.get_pulls.side_effect = lambda state: MagicMock(totalCount=6) if state == "open" else MagicMock(totalCount=2) if state == "closed" else [MagicMock(user=MagicMock(login="dependabot2"))]

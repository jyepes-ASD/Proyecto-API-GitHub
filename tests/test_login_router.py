...
@patch("routers.login_router.get_current_user")
@patch("routers.login_router.Github")
...
@patch("routers.login_router.oauth.github.authorize_redirect")
def test_login(mock_authorize_redirect, mock_request_session):
    mock_authorize_redirect.return_value = Response()
    response = client.get("/login")
    assert response.status_code == status.HTTP_200_OK

@patch("routers.login_router.oauth.github.authorize_access_token")
@patch("routers.login_router.Github")
...
def test_get_current_user_exception(mock_github, mock_request_session):
    ...
    with patch("routers.login_router.get_current_user") as mock_get_current_user:
